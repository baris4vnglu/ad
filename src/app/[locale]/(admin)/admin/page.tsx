...
export default async function AdminDashboard({ params }: Props) {
  ...
  const stats = [
    { label: t("admin.total_users"), value: usersCount ?? 0, color: "blue", icon: Users, href: `/${locale}/admin/users` },
    { label: t("admin.active_jobs"), value: jobsCount ?? 0, color: "emerald", icon: Briefcase, href: `/${locale}/admin/jobs` },
    { label: t("admin.applications"), value: appsCount ?? 0, color: "purple", icon: FileText, href: "#" },
    { label: t("admin.pending"), value: pendingCount ?? 0, color: "amber", icon: Clock, href: `/${locale}/admin/jobs?status=pending` },
    { label: t("admin.pending_payments"), value: pendingPayments ?? 0, color: "purple", icon: CreditCard, href: `/${locale}/admin/payments?status=pending` },
  ];

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-1">{t("admin.panel")}</h1>
      <p className="text-gray-500 mb-8">{t("admin.panel_sub")}</p>
        ...
        {stats.map((s) => (
          <Link key={s.label} href={s.href} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              ...
              {s.label === t("admin.pending") && (s.value as number) > 0 && (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="font-semibold text-gray-900 mb-4">{t("admin.quick_access")}</h2>
